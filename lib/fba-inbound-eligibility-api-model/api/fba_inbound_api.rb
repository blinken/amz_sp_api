...
module AmzSpApi::FbaInboundEligibilityApiModel
  class FbaInboundApi
    ...
    # @param [Hash] opts the optional parameters
    # @option opts [Array<String>] :marketplace_ids The identifier for the marketplace in which you want to determine eligibility. Required only when program&#x3D;INBOUND.
    # @return [Array<(GetItemEligibilityPreviewResponse, Fixnum, Hash)>] GetItemEligibilityPreviewResponse data, response status code and response headers
    def get_item_eligibility_preview_with_http_info(asin, program, opts = {})
      ...
      fail ArgumentError, "Missing the required parameter 'asin' when calling FbaInboundApi.get_item_eligibility_preview" if asin.nil?
      # verify the required parameter 'program' is set
      fail ArgumentError, "Missing the required parameter 'program' when calling FbaInboundApi.get_item_eligibility_preview" if program.nil?
      # verify enum value
      unless ['INBOUND', 'COMMINGLING'].include?(program)
        fail ArgumentError, "invalid value for 'program', must be one of INBOUND, COMMINGLING"
      end
      # resource path
      local_var_path = "/fba/inbound/v1/eligibility/itemPreview".sub('{format}','json')
      ...
      data, status_code, headers = @api_client.call_api(:GET, local_var_path,
        ...
        :return_type => 'GetItemEligibilityPreviewResponse')
      if @api_client.config.debugging
