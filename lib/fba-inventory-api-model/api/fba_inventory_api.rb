  ...
  class FbaInventoryApi
    ...
    def get_inventory_summaries_with_http_info(granularity_type, granularity_id, marketplace_ids, opts = {})
      ...
      if @api_client.config.client_side_validation && marketplace_ids.length > 1
        fail ArgumentError, 'invalid value for "marketplace_ids" when calling FbaInventoryApi.get_inventory_summaries, number of items must be less than or equal to 1.'
      end

      if @api_client.config.client_side_validation && !opts[:'seller_skus'].nil? && opts[:'seller_skus'].length > 50
        fail ArgumentError, 'invalid value for "opts[:"seller_skus"]" when calling FbaInventoryApi.get_inventory_summaries, number of items must be less than or equal to 50.'
      end
