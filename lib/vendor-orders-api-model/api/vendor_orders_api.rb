...
module AmzSpApi::VendorOrdersApiModel
  class VendorOrdersApi
    ...
    # @param purchase_order_number The purchase order identifier for the order that you want. Formatting Notes: 8-character alpha-numeric code.
    # @param [Hash] opts the optional parameters
    # @return [Array<(GetPurchaseOrderResponse, Fixnum, Hash)>] GetPurchaseOrderResponse data, response status code and response headers
    def get_purchase_order_with_http_info(purchase_order_number, opts = {})
      ...
      fail ArgumentError, "Missing the required parameter 'purchase_order_number' when calling VendorOrdersApi.get_purchase_order" if purchase_order_number.nil?
      # resource path
      local_var_path = "/vendor/orders/v1/purchaseOrders/{purchaseOrderNumber}".sub('{format}','json').sub('{' + 'purchaseOrderNumber' + '}', purchase_order_number.to_s)
      ...
      data, status_code, headers = @api_client.call_api(:GET, local_var_path,
        ...
        :return_type => 'GetPurchaseOrderResponse')
      if @api_client.config.debugging
    ...
    # @option opts [String] :purchase_order_state Filters purchase orders based on the purchase order state.
    # @option opts [String] :ordering_vendor_code Filters purchase orders based on the specified ordering vendor code. This value should be same as &#39;sellingParty.partyId&#39; in the purchase order. If not included in the filter, all purchase orders for all of the vendor codes that exist in the vendor group used to authorize the API client application are returned.
    # @return [Array<(GetPurchaseOrdersResponse, Fixnum, Hash)>] GetPurchaseOrdersResponse data, response status code and response headers
    def get_purchase_orders_with_http_info(opts = {})
      ...
      if !opts[:'limit'].nil? && opts[:'limit'] > 100.0
        fail ArgumentError, 'invalid value for "opts[:"limit"]" when calling VendorOrdersApi.get_purchase_orders, must be smaller than or equal to 100.0.'
      end

      if !opts[:'limit'].nil? && opts[:'limit'] < 1.0
        fail ArgumentError, 'invalid value for "opts[:"limit"]" when calling VendorOrdersApi.get_purchase_orders, must be greater than or equal to 1.0.'
      end

      if opts[:'sort_order'] && !['ASC', 'DESC'].include?(opts[:'sort_order'])
        fail ArgumentError, 'invalid value for "sort_order", must be one of ASC, DESC'
      end
      if opts[:'po_item_state'] && !['Cancelled'].include?(opts[:'po_item_state'])
        fail ArgumentError, 'invalid value for "po_item_state", must be one of Cancelled'
      end
      if opts[:'purchase_order_state'] && !['New', 'Acknowledged', 'Closed'].include?(opts[:'purchase_order_state'])
        fail ArgumentError, 'invalid value for "purchase_order_state", must be one of New, Acknowledged, Closed'
      end
      # resource path
      local_var_path = "/vendor/orders/v1/purchaseOrders".sub('{format}','json')
      ...
      data, status_code, headers = @api_client.call_api(:GET, local_var_path,
        ...
        :body => post_body,
        :auth_names => auth_names,
        :return_type => 'GetPurchaseOrdersResponse')
      if @api_client.config.debugging
    ...
    # @option opts [String] :ordering_vendor_code Filters purchase orders based on the specified ordering vendor code. This value should be same as &#39;sellingParty.partyId&#39; in the purchase order. If not included in filter, all purchase orders for all the vendor codes that exist in the vendor group used to authorize API client application are returned.
    # @option opts [String] :ship_to_party_id Filters purchase orders for a specific buyer&#39;s Fulfillment Center/warehouse by providing ship to location id here. This value should be same as &#39;shipToParty.partyId&#39; in the purchase order. If not included in filter, this will return purchase orders for all the buyer&#39;s warehouses used for vendor group purchase orders.
    # @return [Array<(GetPurchaseOrdersStatusResponse, Fixnum, Hash)>] GetPurchaseOrdersStatusResponse data, response status code and response headers
    def get_purchase_orders_status_with_http_info(opts = {})
      ...
      if !opts[:'limit'].nil? && opts[:'limit'] > 100.0
        fail ArgumentError, 'invalid value for "opts[:"limit"]" when calling VendorOrdersApi.get_purchase_orders_status, must be smaller than or equal to 100.0.'
      end

      if !opts[:'limit'].nil? && opts[:'limit'] < 1.0
        fail ArgumentError, 'invalid value for "opts[:"limit"]" when calling VendorOrdersApi.get_purchase_orders_status, must be greater than or equal to 1.0.'
      end

      if opts[:'sort_order'] && !['ASC', 'DESC'].include?(opts[:'sort_order'])
        fail ArgumentError, 'invalid value for "sort_order", must be one of ASC, DESC'
      end
      if opts[:'purchase_order_status'] && !['OPEN', 'CLOSED'].include?(opts[:'purchase_order_status'])
        fail ArgumentError, 'invalid value for "purchase_order_status", must be one of OPEN, CLOSED'
      end
      if opts[:'item_confirmation_status'] && !['ACCEPTED', 'PARTIALLY_ACCEPTED', 'REJECTED', 'UNCONFIRMED'].include?(opts[:'item_confirmation_status'])
      ...
      if opts[:'item_receive_status'] && !['NOT_RECEIVED', 'PARTIALLY_RECEIVED', 'RECEIVED'].include?(opts[:'item_receive_status'])
        fail ArgumentError, 'invalid value for "item_receive_status", must be one of NOT_RECEIVED, PARTIALLY_RECEIVED, RECEIVED'
      end
      # resource path
      local_var_path = "/vendor/orders/v1/purchaseOrdersStatus".sub('{format}','json')
      ...
      data, status_code, headers = @api_client.call_api(:GET, local_var_path,
        ...
        :body => post_body,
        :auth_names => auth_names,
        :return_type => 'GetPurchaseOrdersStatusResponse')
      if @api_client.config.debugging
    ...
    def submit_acknowledgement_with_http_info(body, opts = {})
      ...
      # resource path
      local_var_path = "/vendor/orders/v1/acknowledgements".sub('{format}','json')
